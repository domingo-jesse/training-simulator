...
from utils import inject_scroll_to_top, inject_styles, page_container, render_page_header, render_user_top_bar
...
def render_main_app() -> None:
    ...
    header_action = render_user_top_bar(
        display_name=(user.get("full_name") or user.get("email") or "User"),
        email=user.get("email") or "",
        key_prefix="main_header",
    )
    if header_action == "profile":
        st.session_state["page"] = "Profile"
        st.session_state["current_page"] = "profile"
        _set_nav("profile")
        st.rerun()
    if header_action == "settings":
        st.session_state["page"] = "Settings"
        st.session_state["current_page"] = "settings"
        _set_nav("settings")
        st.rerun()
    if header_action == "sign_out":
        logout_user()
        st.rerun()
