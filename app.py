...
def _render_sticky_user_header() -> None:
    ...
    st.markdown('<div class="sticky-user-header-marker" aria-hidden="true"></div>', unsafe_allow_html=True)
    _, user_text_col, menu_col = st.columns([9.4, 1.45, 0.45], vertical_alignment="center")
    with user_text_col:
        st.markdown(
            f"""
            <div class="admin-user-text fixed-user-text">
    ...
    with menu_col:
        st.markdown('<div class="settings-button-wrap fixed-user-menu">', unsafe_allow_html=True)
...
def render_main_app() -> None:
    ...
    st.markdown(
        ...
        .sticky-user-header-marker {
            display: none;
        }
        [data-testid="stVerticalBlock"]:has(.sticky-user-header-marker) [data-testid="column"] {
            padding-top: 0 !important;
            padding-bottom: 0 !important;
        }
        [data-testid="stVerticalBlock"]:has(.sticky-user-header-marker) .fixed-user-text {
            margin: 0;
            line-height: 1.05;
        }
        [data-testid="stVerticalBlock"]:has(.sticky-user-header-marker) .fixed-user-menu {
            display: flex;
            justify-content: flex-end;
        }
        </style>
